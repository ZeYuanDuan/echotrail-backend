...
- [ ] **Step 3: Document Console sequence.** Create the Cloud SQL instance/database; create `echotrail_migrator` (schema owner) and `echotrail_app` (DML only); create two Secret Manager secrets with those passwords; create `echotrail-db-migrator@echotrail-dev-508500-k6.iam.gserviceaccount.com` and grant Cloud SQL Client plus accessor only to migration secret; grant backend runtime account Cloud SQL Client plus accessor only to app secret; grant Cloud Build deploy identity Cloud Run Admin at project level (includes job execution), Service Account User for both runtime identities, and Artifact Registry writer/reader. Explain that database grants are separate from IAM, and show concrete SQL: `GRANT CONNECT ON DATABASE echotrail TO echotrail_app; GRANT USAGE ON SCHEMA public TO echotrail_app; GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO echotrail_app; GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO echotrail_app; ALTER DEFAULT PRIVILEGES FOR ROLE echotrail_migrator IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO echotrail_app; ALTER DEFAULT PRIVILEGES FOR ROLE echotrail_migrator IN SCHEMA public GRANT USAGE, SELECT ON SEQUENCES TO echotrail_app;`. Run grants after the first migration, or default privileges before and existing grants afterward. Grant migrator `CREATE` on public schema if PostgreSQL version/database defaults do not. Do not grant app DDL.
